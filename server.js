...
// quand un client se connecte le serveur recoit l'event connexion
// "on" c'est un peut comme "addeventlistener"

io.on("connection", (ws) => {
  // ici le handler de "connexion"
  console.log(">> socket.io - connected");

  // le serveur se met à l'ecoute de "client_send_message"
  ws.on("client_send_message", (message) => {
    // ici le hanlder de l'event "client_send_message"

    // il ajoute un id au message reçu
    // eslint-disable-next-line no-plusplus
    message.id = ++id;
    console.log("message reçu", message);

    // le serveur emmet l'event "server_send_message" avec le message :
    // il le broadcast à tous les clients connectés
